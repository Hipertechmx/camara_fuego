...
void apply_debanding_filter(float *pixels, int cols, int rows) {
  for (int y = 1; y < rows - 1; y++) {
    for (int x = 1; x < cols - 1; x++) {
      ...
      float avg = (up + down) / 2.0;
      float diff = fabs(current - avg);
      if (diff < 0.5) {
        float smoothed = (current + avg) / 2.0;
        set_point(pixels, rows, cols, x, y, smoothed);
      }
    }
  }
}
